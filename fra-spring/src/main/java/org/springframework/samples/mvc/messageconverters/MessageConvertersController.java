...
public class MessageConvertersController {
    ...
    // Jaxb2RootElementHttpMessageConverter (requires JAXB2 on the classpath - useful for serving
    // clients that expect to work with XML)
    ...
    // MappingJacksonHttpMessageConverter (requires Jackson on the classpath - particularly useful
    // for serving JavaScript clients that expect to work with JSON)

    @RequestMapping(value = "/json", method = RequestMethod.POST)
    public
    @ResponseBody
    String readJson(@Valid @RequestBody JavaBean bean) {
    ...
    // RssChannelHttpMessageConverter (requires Rome on the classpath - useful for serving RSS
    // feeds)
